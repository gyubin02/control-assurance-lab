...
export async function executeElasticVerification(argv) {
  ...
  try {
    ...
    const [receipt, expectedRequest] = await Promise.all([
      readPinnedRegularFile(
        args["--receipt"],
        ELASTIC_RECEIPT_LIMITS.maxBytes,
      ),
      readPinnedRegularFile(
    ...
    const verification = verifyElasticSecurityCapture(receipt, {
    ...
    return {
      ...
      stdout: Buffer.concat([
        canonicalize(
          elasticVerificationSummary(verification, {
...
async function main() {
  const outcome = await executeElasticVerification(process.argv.slice(2));
  if (outcome.exitCode === 2) {
    process.stderr.write(
      "usage: elastic-capture-verify --receipt FILE --expected-request FILE " +
        "--expected-endpoint-origin-digest sha256:HEX " +
        "--expected-connector-version VERSION [--records-output NEW_FILE]\n",
    );
  } else {
